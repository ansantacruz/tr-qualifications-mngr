...
export default class SellerQualificationDataSource {
    ...
    public static readonly getSellerQualification = async (idSeller : number):
    Promise<ISellerQualificationResponse[]> => {
        debug('Starts the database query of the search configuration');
        try {
            const rqUid = 'test';
            const result = await executeSQL(
                `select
                    ven_id as idVendedor,
                    CONCAT(venUsu.usu_nombre, ' ', venUsu.usu_apellido) as nombreVendedor,
                    com_id as idComprador , CONCAT(comUsu.usu_nombre, ' ',
                    comUsu.usu_apellido) as nombreComprador ,
                    cve_descripcion as comentario ,
                    tca_descripcion as calificacion ,
                    tca_valor as porcentajeCalificacion,
                    DATE(cve_fechaCreacion) as fechaComentario
    ...
    public static readonly addSellerQualification = async (dataRequest : ISellerAddQualificationRequest):
    Promise<ISellerAddQualificationResponse> => {
        try {
            const idVendedor = dataRequest.idVendedor;
            const idComprador = dataRequest.idComprador;
            const idTipoComentario = dataRequest.idTipoComentario;
            const comentario = dataRequest.comentario;
            const rqUid = 'test';
            const result = await executeSQL(
                `INSERT INTO tr_data_base.calificacion_vendedor (
                    cve_vendedor,
                    cve_descripcion,
                    cve_tipoCalificacion,
                    cve_fechaCreacion,
                    cve_comprador,
                    cve_activo) VALUES ($idVendedor, $comentario, $idTipoComentario,  now(), $idComprador, '1');`,
                QueryTypes.INSERT,
